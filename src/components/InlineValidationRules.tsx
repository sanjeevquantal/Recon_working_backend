...
const InlineValidationRules: React.FC<InlineValidationRulesProps> = ({
  ...
  const availableFields = matchedFields.map(match => match.field1);

  const handleAddRule = () => {
    form.handleSubmit((data) => {
      // Create a properly typed ValidationRule
  ...
  // Field2 is now available for all rule types, removing this check
  // const needsSecondField = (operator: string) => {
  //  return operator === 'compare_fields' || operator === 'sum_equals';
  // };
        ...
        <Form {...form}>
          <div className="space-y-6">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-sm">Field 1</FormLabel>
                  <FormControl>
            ...
            {/* Field 2 - Always visible now for all rule types */}
            <FormField
              control={form.control}
              name="field2"
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-sm">Field 2</FormLabel>
                  <FormControl>
                    <Select onValueChange={field.onChange} value={field.value}>
                      <SelectTrigger className="w-full bg-gray-700/30">
